...

{
	...
	//Calibration info
	Camera_K = 600, 0., IMAGE_RESOLUTION_X / 2,
		0., 6.5746697810243404e+002, IMAGE_RESOLUTION_Y,
		0., 0., 1.;
	discoeff = 0., 0., 0., 0., 0.;
	...
	//Read in Image data
	this->ImgDataSeq = (unsigned char*)malloc(sizeof(unsigned char) * IMAGE_RESOLUTION_X * IMAGE_RESOLUTION_Y * numImages * 3);
	// Then grab the data
...
Mat LightfieldClass::RenderImage(vector<Point3d> proxyPoint, Point3d VirtualCameraLoc, 
							     vector<Point3d> AllCameraLocs, Matx33d VirtualRot, 
								 Vec3d VirtualTrans)
{
	...
	for (unsigned int numP = 0; numP<proxyPoint.size(); numP++)
	{
		...
		for (int i = 0; i<this->kth; i++)
		{
			...
			unsigned char curimgIntensity_R = this->ImgDataSeq[IMAGE_RESOLUTION_X*IMAGE_RESOLUTION_Y * 3 * Num + (int)curimgPoint[0].y*IMAGE_RESOLUTION_X * 3 + (int)curimgPoint[0].x];
			unsigned char curimgIntensity_G = this->ImgDataSeq[IMAGE_RESOLUTION_X*IMAGE_RESOLUTION_Y * 3 * Num + (int)curimgPoint[0].y*IMAGE_RESOLUTION_X * 3 + (int)curimgPoint[0].x + 1];
			unsigned char curimgIntensity_B = this->ImgDataSeq[IMAGE_RESOLUTION_X*IMAGE_RESOLUTION_Y * 3 * Num + (int)curimgPoint[0].y*IMAGE_RESOLUTION_X * 3 + (int)curimgPoint[0].x + 2];
